...
backed by localStorage. Streamlit's session_state plays the same role; the
expensive part (valuing 200 products) is memoised on a hash of the settings
that actually affect the numbers.
"""
...
def init() -> None:
    ...
    if "products" not in ss:
        ss.products = build_catalog_products(ss.settings)
...
def reset_demo() -> None:
    st.session_state.products = build_catalog_products(st.session_state.settings)
